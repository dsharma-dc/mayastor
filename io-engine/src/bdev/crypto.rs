...
use std::{ffi::CStr, fmt::{Debug, Display, Formatter}};
use nix::errno::Errno;
use futures::channel::oneshot;
use spdk_rs::{ffihelper::IntoCString, libspdk::{
    spdk_accel_assign_opc,
    accel_dpdk_cryptodev_enable,
    accel_dpdk_cryptodev_set_driver,
    create_crypto_disk,
    delete_crypto_disk,
...
use crate::{bdev_api::BdevError, core::CoreError, ffihelper::{cb_arg, pair, FfiResult}};
...
/// AES_XTS. We don't expect to use default though without inputs from upstream.
pub enum Cipher {
    AesCbc,
    #[default]
    AesXts,
...
impl Debug for Crypto {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "crypto vbdev '{}', 'base_bdev: {}', keyname '{}', key '{}', key2 '{:?}'",
        self.name, self.base_bdev_name, self.key.key_name, self.key.key, self.key.key2)
    }
}
...
pub fn enable_dpdk_cryptodev_scan_accel_module() -> Result<(), CoreError> {
    ...
    unsafe {
        ...
        if setdrv_ret != 0 {
            return Err(CoreError::InitCryptoModule { reason: format!("Unsupported driver: {:?}", setdrv_ret)});
        }
        ...
        if err_enc != 0 || err_dec != 0 {
            return Err(
                CoreError::InitCryptoModule {
                    reason: format!("opcode assignment failed. err_enc {err_enc:?}, err_dec {err_dec:?}")
                }
            );
        }
        Ok(())
...
fn create_crypto_key(
    ...
    key_name_str: &str,
    hex_key_str: &str,
    hex_key2_str: Option<&str>,
) -> Result<*mut spdk_accel_crypto_key, BdevError> {
    let cipher = cipher.to_string().into_cstring().into_raw();
    let key = hex_key_str.into_cstring().into_raw();
    let key2 = if let Some(k2) = hex_key2_str {
        k2.into_cstring().into_raw()
    } else {
        std::ptr::null_mut()
    };
    let key_name = key_name_str.into_cstring().into_raw();

    unsafe {
        debug!("Creating {} cipher key {} | {k2} of size {}",
            CStr::from_ptr(cipher).to_str().unwrap(),
            CStr::from_ptr(key).to_str().unwrap(),
            hex_key_str.as_bytes().len() * 4,
            k2 = if hex_key2_str.is_some() { CStr::from_ptr(key2).to_str().unwrap() } else { "" },
        );
    }
    
    let key_params = spdk_accel_crypto_key_create_param {
        cipher: cipher,
        hex_key: key,
        hex_key2: key2,
        key_name: key_name,
    ...
    unsafe {
        let ret = spdk_accel_crypto_key_create(&key_params);
        if ret != 0 {
            Err(BdevError::CreateBdevFailed {
                source: Errno::from_raw(ret.abs()),
                name: format!("Key create failed: {:?}", key_params.key_name)
            })
        } else {
            let key_ptr = spdk_accel_crypto_key_get(key_params.key_name);
            trace!("Key '{key_name_str}' created." );
            Ok(key_ptr)
        }
    }
}
...
pub async fn destroy_crypto_vbdev(vbdev_name: &str) -> Result<(), BdevError> {
    let (s, r) = pair::<i32>();
    unsafe {
        delete_crypto_disk(
    ...
    info!("{vbdev_name}: crypto vbdev destroyed successfully");
    Ok(())
}
...
pub fn create_crypto_vbdev_on_base_bdev(
    ...
    // Create the key.
    let key = create_crypto_key(
        key_params.cipher,
        &key_params.key_name,
        &key_params.key,
        key_params.key2.as_ref().map(|k2| k2.as_str()),
    )?;

    // Setup the crypto opts using the key now.
    let crypto_opts_ptr = unsafe {
        create_crypto_opts_by_name(
            crypto_vbdev_name.into_cstring().into_raw(),
            base_bdev_name.into_cstring().into_raw(),
            key,
            true,
        )
    };
    ...
    let ret = unsafe {
        create_crypto_disk(crypto_opts_ptr)
    };
    
    if ret != 0 {
        Err(BdevError::CreateBdevFailed { source: Errno::from_raw(ret),name: crypto_vbdev_name.to_string()})
    } else {
